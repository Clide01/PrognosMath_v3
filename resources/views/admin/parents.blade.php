<x-app-layout>
    <div class="py-10 bg-slate-50 min-h-screen" x-data="{ 
        ...
        openEditModal: false, 
        editId: '', editFirstName: '', editLastName: '', editEmail: '', editContactNumber: '' 
    }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200/80 flex justify-between items-center">
                <div>
                    <h2 class="text-2xl font-black text-slate-900">Parent Accounts</h2>
                    <p class="text-slate-500 text-sm mt-1 font-medium">Manage parent accounts and verify contact details.</p>
                </div>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($parents ?? [] as $parent)
                                <tr class="hover:bg-slate-50 transition">
                                    <td class="p-5 font-extrabold text-slate-800 text-sm">{{ $parent->first_name }} {{ $parent->last_name }}</td>
                                    <td class="p-5 text-sm font-semibold text-slate-600">{{ $parent->email }}</td>
                                    <td class="p-5 text-sm font-mono font-bold text-blue-600">{{ $parent->contact_number ?? 'Not Provided' }}</td>
                                    <td class="p-5 text-center space-x-3">
                                        <button @click="
                                            openEditModal = true;
                                            editId = '{{ $parent->id }}';
                                            editFirstName = '{{ addslashes($parent->first_name) }}';
                                            editLastName = '{{ addslashes($parent->last_name) }}';
                                            editEmail = '{{ $parent->email }}';
                                            editContactNumber = '{{ $parent->contact_number }}';
                                        " class="text-blue-600 hover:text-blue-800 text-xs font-bold transition">Edit</button>
                                        
                                        <form method="POST" action="{{ route('admin.users.destroy', $parent->id) }}" class="inline" onsubmit="return confirm('Delete this parent account permanently?');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="p-10 text-center text-slate-500 font-bold text-sm">No parents linked yet.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div x-show="openCreateModal" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm overflow-y-auto" x-cloak style="display: none;">
                <div class="flex min-h-full items-center justify-center p-4">
                    <div class="bg-white rounded-3xl max-w-md w-full p-8 shadow-2xl border border-slate-100" @click.away="openCreateModal = false">
                        <h3 class="text-xl font-black text-slate-900 mb-6">Register New Parent</h3>
                        <form method="POST" action="{{ route('admin.parents.store') }}" class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="contact_number" class="w-full rounded-xl border-slate-200 text-sm font-semibold p-3 focus:border-slate-900 focus:ring-slate-900" placeholder="09171234567" required>
                            </div>
                            <div>
                            ...
                            <div class="flex justify-end gap-3 pt-4 mt-6 border-t border-slate-100">
                                ...
                                <button type="submit" class="px-6 py-2.5 text-xs bg-slate-900 text-white hover:bg-blue-600 rounded-xl font-bold transition shadow-sm">Register Parent</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div x-show="openEditModal" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm overflow-y-auto" x-cloak style="display: none;">
                <div class="flex min-h-full items-center justify-center p-4">
                    <div class="bg-white rounded-3xl max-w-md w-full p-8 shadow-2xl border border-slate-100" @click.away="openEditModal = false">
                        <h3 class="text-xl font-black text-slate-900 mb-6">Edit Parent Details</h3>
                        <form method="POST" :action="`/admin/users/${editId}`" class="space-y-4">
                            ...
                            <div>
                                <label class="block text-xs font-extrabold text-slate-500 uppercase tracking-wider mb-1.5">Mobile Number</label>
                                <input type="text" name="contact_number" x-model="editContactNumber" class="w-full rounded-xl border-slate-200 text-sm font-semibold p-3 focus:border-slate-900 focus:ring-slate-900" required>
                            </div>
